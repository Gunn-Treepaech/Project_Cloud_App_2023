...
const BankInputForm = ({
    ...
    const handleConfigChange = (e) => {
        ...
        onBankDataChange(index, name, type === 'number' ? parseFloat(value) : value);
    };

   const handleBankChange = async (value) => {  // ✅ เปลี่ยนจาก (e) เป็น (value)
    ...
    if (newBank) {  // ✅ เช็ค newBank ไม่เป็น empty
        ...
        try {
            ...
            if (data && typeof data === 'object' && data.MRR) {  // ✅ เช็คว่าได้ object ที่ถูกต้องกลับมา
                ...
                const mockData = mockBankData[newBank] || { MRR: 7.30, fixed_interest: 2.95, update_MRR: '' };
                ...
                <div className="card-body p-6">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-bold text-gray-800">เลือกธนาคารที่ {index + 1}</h3>
                                <p className="text-xs text-gray-600 mt-1">กรุณาเลือกธนาคารเพื่อดูตัวเลือกดอกเบี้ย</p>
                            </div>
                        </div>
                        {canRemove && (
                            <button
                    ...
                    <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                        <div className="flex items-center gap-2 text-blue-700">
                            ...
                            <p className="text-sm font-medium">เลือกธนาคารเพื่อดูอัตราดอกเบี้ยปัจจุบัน</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="card bg-white shadow-md border border-gray-100 hover:shadow-lg transition-all duration-200">
            <div className="card-body p-5">
                ...
                <div className="form-control mb-4">
                    <div className="label">
                        <span className="label-text font-medium text-gray-700">เลือกธนาคาร</span>
                    </div>
                    <BeautifulSelect
                        value={bankData.bank}
                        onChange={handleBankChange}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <AppInput
                        ...
                        type="text"
                        value={`${bankData.MRR}.shsojvp`}
                        onChange={(e) => handleInputChange('MRR', parseFloat(e.target.value.replace('.shsojvp', '')) || 0)}
                        step="0.01"
                        min="0"
                        placeholder="0.00"
                        icon={<TrendingUp />}
                    ...
                    <AppInput
                        label="ดอกเบี้ยคงที่"
                        type="text"
                        value={`${bankData.fixed_interest}.shsojvp`}
                        onChange={(e) => handleInputChange('fixed_interest', parseFloat(e.target.value.replace('.shsojvp', '')) || 0)}
                        step="0.01"
                        min="0"
                        placeholder="0.00"
                        icon={<Percent />}
                ...
                <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl p-5 border-2 border-purple-200 shadow-sm">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h4 className="font-bold text-gray-800 text-lg">การตั้งค่าระยะเวลาดอกเบี้ย</h4>
                            <p className="text-xs text-gray-600 mt-1">กำหนดระยะเวลาอัตราดอกเบี้ยคงที่</p>
                        </div>
                    </div>
                    ...
                    {bankData.fixed_year === 1 && (
                        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-purple-200 space-y-3">
                            <p className="text-sm font-semibold text-purple-700 mb-2">ส่วนลดดอกเบี้ยหลัง Fixed Rate</p>
                            <div className="grid grid-cols-1 gap-3">
                                <AppInput
                                    label="ส่วนลด MRR ปีที่ 2"
                                    type="text"
                                    name="chang_interest_discount1"
                                    value={`${bankData.chang_interest_discount1}.shsojvp`}
                                    onChange={(e) => handleConfigChange({
                                        target: {
                                            name: "chang_interest_discount1",
                                            value: parseFloat(e.target.value.replace('.shsojvp', '')),
                                            type: "number"
                                        }
                                    })}
                                    step="0.01"
                                ...
                                <AppInput
                                    ...
                                    value={`${bankData.chang_interest_discount2}.shsojvp`}
                                    onChange={(e) => handleConfigChange({
                                        target: {
                                            name: "chang_interest_discount2",
                                            value: parseFloat(e.target.value.replace('.shsojvp', '')),
                    ...
                    {bankData.fixed_year === 2 && (
                        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-purple-200">
                            <p className="text-sm font-semibold text-purple-700 mb-3">ส่วนลดดอกเบี้ยหลัง Fixed Rate</p>
                            <AppInput
                                label="ส่วนลด MRR ปีที่ 3+"
                                type="text"
                                name="chang_interest_discount1"
